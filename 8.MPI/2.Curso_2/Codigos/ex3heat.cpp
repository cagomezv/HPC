...
static char help[] ="Solves a simple time-dependent linear PDE \
                  (the heat equation).\n\
...
typedef struct {
  ...
  PetscTruth  debug;             /* flag (1 indicates activation 
...
extern PetscErrorCode RHSMatrixHeat(TS,PetscReal,Mat*,
...
int main(int argc,char **argv)
{
  ...
  ierr = PetscOptionsHasName(PETSC_NULL,"-debug",
                             &appctx.debug);CHKERRQ(ierr);    
  appctx.m        = m;
  appctx.h        = 1.0/(m-1.0);
  appctx.norm_2   = 0.0;
  appctx.norm_max = 0.0;
  ierr = PetscPrintf(PETSC_COMM_SELF,"Solving a linear TS problem "
                     "on 1 processor\n");CHKERRQ(ierr);
  ...
  */
  ...
  ierr = PetscViewerDrawOpen(PETSC_COMM_SELF,0,"",80,380,400,160,
                             &appctx.viewer1);CHKERRQ(ierr);
  ierr = PetscViewerDrawGetDraw(appctx.viewer1,0,&draw);CHKERRQ(ierr);
  ierr = PetscDrawSetDoubleBuffer(draw);CHKERRQ(ierr);   
  ierr = PetscViewerDrawOpen(PETSC_COMM_SELF,0,"",80,0,400,160,
                             &appctx.viewer2);CHKERRQ(ierr);
  ...
  ierr = PetscOptionsHasName(PETSC_NULL,
                             "-time_dependent_rhs",&flg);CHKERRQ(ierr);
  if (flg) {
    /*
    ...
    */
    ierr = TSSetMatrices(ts,A,RHSMatrixHeat,PETSC_NULL,
                         PETSC_NULL,DIFFERENT_NONZERO_PATTERN,
                         &appctx);CHKERRQ(ierr);
  } else {
    /*
...
PetscErrorCode InitialConditions(Vec u,AppCtx *appctx)
{
  ...
  */
  for (i=0; i<appctx->m; i++) {
    u_localptr[i] = PetscSinScalar(PETSC_PI*i*6.*h) +
...
*/
PetscErrorCode ExactSolution(PetscReal t,Vec solution,
                             AppCtx *appctx) {
...
/*
   ...
   ctx    - the user-provided context for this monitoring routine.
            ...
            solution.
*/
PetscErrorCode Monitor(TS ts,PetscInt step,PetscReal time,
                       Vec u,void *ctx) {
  ...
  */
  ...
  ierr = PetscPrintf(PETSC_COMM_WORLD,
                     "Timestep %3D: step size = %-11g, time = %-11g,"
                     " 2-norm error = %-11g, max norm error = %-11g\n",
                     step,dt,time,norm_2,norm_max);CHKERRQ(ierr);
  appctx->norm_2   += norm_2;
  appctx->norm_max += norm_max;

  dttol = .0001;
  ierr = PetscOptionsGetReal(PETSC_NULL,"-dttol",&dttol,
...
*/
PetscErrorCode RHSMatrixHeat(TS ts,PetscReal t,Mat *AA,Mat *BB,
